...
namespace minila {
    template<typename Type = float> requires std::floating_point<Type>
    class BaseArray {
    public:
        friend class BaseArray;

        BaseArray() : _values(nullptr), _dimensions(nullptr) {};
    ...
    private:
        ...
        std::unique_ptr<std::vector<Type>> _values;
        std::unique_ptr<std::vector<uint64_t>> _dimensions;
    };

    template<typename Type>
    requires std::floating_point<Type>
    BaseArray<Type>::BaseArray(const BaseArray<Type> &right) {
        _dimensions = std::make_unique<std::vector<uint64_t>>(*(right._dimensions));
        _values = std::make_unique<std::vector<Type>>(*(right._values));
    }
    ...
    BaseArray<Type>::BaseArray(const std::vector<uint64_t> &dimensions) {
        ...
        _dimensions = std::make_unique<std::vector<uint64_t>>(dimensions);
        _values = std::make_unique<std::vector<Type>>(std::vector<Type>(N, 0.0));
    }
    ...
    BaseArray<Type>::BaseArray(const std::vector<uint64_t> &dimensions, const std::vector<Type> &values) {
        _dimensions = std::make_unique<std::vector<uint64_t>>(dimensions);
        _values = std::make_unique<std::vector<Type>>(values);
    }
